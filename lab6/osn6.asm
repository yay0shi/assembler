...
;Таблица дескрипторов прерываний IDT.
 ...
 ex16    Idt_Descriptor<offset ex16_proc,cs_code,0,10000111b,0>
         Idt_Descriptor 13 dup(<>)
 Int30   Idt_Descriptor<offset int30_proc,cs_code,0,10000110b,0> ; Новый обработчик
         Idt_Descriptor 8 dup(<>)
...
ErrorMsg          db  'Invalid selector$'
LimitMsg           db  'Segment Limit: $'  ; Сообщение о размере сегмента
LimitBuffer        db  9 dup(' '), '$'     ; Буфер для числа
;Len                dw  14d						-
...
;**************************************************************
; Обработчик прерывания 30d - выводит размер сегмента
; Вход: DX - селектор дескриптора в GDT
;**************************************************************
Int30_Proc   Proc  Near
    pusha
    push    ds
    push    es
    
    ; Используем селектор данных для доступа к переменным
    mov     ax, Cs_Data
    mov     ds, ax
    
    ; Проверяем корректность селектора
    test    dx, 00000100b    ; Проверяем TI-бит (бит 2)
    jnz     Int30_Error      ; Если TI=1 (LDT) - ошибка
    
    mov     bx, dx
    and     bx, 0FFF8h       ; Очищаем RPL и TI, оставляем индекс
    cmp     bx, Gdt_Leng - 8 ; Проверяем границы GDT
    jae     Int30_Error
    
    ; В защищенном режиме для доступа к GDT нужно использовать
    ; специальные инструкции или заранее подготовленные данные
    ; Используем инструкции защищенного режима для получения лимита
    
    mov     ax, dx           ; Загружаем селектор в AX
    lar     ecx, eax         ; Получаем атрибуты дескриптора
    jnz     Int30_Error      ; Если невалидный селектор
    
    lsl     edx, eax         ; Получаем лимит сегмента
    jnz     Int30_Error      ; Если ошибка
    
    ; EDX содержит лимит сегмента
    ; Проверяем бит гранулярности
    test    ch, 80h          ; Проверяем G-бит (бит 7 старшего байта атрибутов)
    jz      No_Granularity
    
    ; G=1 - гранулярность страниц (4KB)
    inc     edx              ; Лимит + 1
    shl     edx, 12          ; × 4096
    dec     edx              ; Получаем реальный размер - 1
    jmp     Convert_Size
    
No_Granularity:
    ; G=0 - байтовая гранулярность
    ; Размер уже правильный (максимальное смещение)
    inc     edx              ; Реальный размер = лимит + 1
...
Convert_Size:
    ...
    mov     ax, Video_Desc
    mov     es, ax
    mov     bx, offset LimitMsg
    mov     dx, 0C0Fh     ; Строка 12, колонка 0
    call    PrintString
    
    ; Выводим значение лимита
    mov     bx, offset LimitBuffer
    mov     dx, 0C10h     ; Строка 12, колонка 16
    call    PrintString
...
;**************************************************************
; Преобразование двойного слова в EAX в шестнадцатеричную строку
;**************************************************************
DwordToHex PROC
    pusha
    mov     di, offset LimitBuffer
    mov     ecx, eax
...
;**************************************************************
; Вывод строки на экран в защищенном режиме
; Вход: DS:BX - адрес строки
;       DX - позиция (DH-строка, DL-колонка)
;       ES - селектор видеопамяти
;**************************************************************
PrintString PROC
           pusha
           xor     cx,cx
...
PrintString_Loop:
           ...
           cmp     al,'$'
           jz      PrintString_Exit
           mov     ah,07h           ; Атрибут - светло-серый на черном
           mov     es:[di],ax
           add     di,2
           inc     bx
...
;          mov     ds,ax             ; DS - сегмент данных		-
           ...
           ; Тестируем обработчик прерывания 30d
           mov     dx, Cs_Code      ; Селектор кодового сегмента
           int     30d
